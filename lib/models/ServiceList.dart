...
class ServiceList {
  ...
  Future<double> _calculateDistance(Service service, Position position) async {
    return await Geolocator().distanceBetween(service.latitude,
        service.longitude, position.latitude, position.longitude);
  }

  // Sorted by distance: from near to far
  Future<void> sortByDistance(Position position) async {
    for (int i = 0; i < services.length; ++i) {
      this.services[i].distance =
          await _calculateDistance(services[i], position);
    }
